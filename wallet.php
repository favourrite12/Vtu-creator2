...
<title>
<?php echo $LANG['transactions_paid_with_wallet']; ?>
</title>
  ...
  <section id="content">
          ...
          <div class="container">
            <div class="section">
			  ...
			  <h4><?php echo $LANG["transactions_paid_with_wallet"]; ?> </h4>
<?php echo $LANG['the_below_table_contains_payment_history_of_transactions_paid_with_wallet']; ?>
...
$totalQuery = $conn->query("SELECT id FROM recharge WHERE payment_method='wallet'")->num_rows;
$page->searchForm($action);
$pageData = $page->getData($currentPage,$totalQuery);
$start = $pageData["start"];
$stop = $pageData["stop"];

$sql = "SELECT amount,id,payment_method,status,service_id FROM recharge  WHERE payment_method='wallet' ORDER BY reg_date DESC LIMIT $start,$stop";
