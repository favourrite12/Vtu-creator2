...
<title>
<?php echo $LANG['api_transactions']; ?>-<?php echo $LANG['failed_transactions']; ?>
</title>
  ...
  <section id="content">
          ...
          <div class="container">
            <div class="section">
			  ...
			  <h4><?php echo $LANG['api_transactions']; ?> - <?php echo $LANG["failed_transactions"]; ?> </h4>
...
$totalQuery = $conn->query("SELECT id FROM api_transaction WHERE status='failed'")->num_rows;
$page->searchForm($action);
$pageData = $page->getData($currentPage,$totalQuery);
$start = $pageData["start"];
$stop = $pageData["stop"];

$sql = "SELECT amount,id,status,service_id FROM api_transaction  WHERE status='failed' ORDER BY reg_date DESC LIMIT $start,$stop";
