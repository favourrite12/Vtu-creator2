...
$transactionValue["type"] = "wallet";
	//print_r($transactionValue);
$user = $transactionValue["owner"];	
$conn->query("UPDATE payment SET  payment_method_id='$method' WHERE id='$id'");
$changeMethod = "method.php?wallet=$id";
}
?>
<?php include '../account/userinfojson.php';?>
 ...
 if(!empty($_GET["recharge"])){
     ...
     $serviceValue["display_name"]=$LANG["wallet_funding"];
     $serviceValue["fee"]=$webConfig["walletCharge"];
     $serviceValue["fee_capped"]=$webConfig["chargeCap"];
     $serviceValue["fee_percentage"]=$webConfig["walletFeePercentage"];
 }			
?>
...
if(empty($userInfo['name'])){
	...
	$userInfo['email'] = empty($transactionValue["email"])?$webConfig["supportEmail"]:$transactionValue["email"];
}
