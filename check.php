...
<title><?php echo $LANG["check_website_update"]; ?></title>
...
<?php 
		...
		$host = 'https://update.lajela.com/provtu2/getupdate.php';
	        include "var.php";
		...
		$message = '<div class="alert alert-success alert-dismissable mt-5">
					 ...
					 <strong> '.$LANG["congratulations_your_website_is_up_to_date"].' <i class="fa fa-check" ></i></strong>. 
					 
					</div>
					<a class="btn btn-success right" href="index.php">'.$LANG["go_back"].'</a>';
		}else{
			$_SESSION["updates"] = $updates;
		     $btn = '<a class="btn btn-success left" href="start.php">'.$LANG["install_update"].'</a>
			 <a class="btn btn-info right" href="updateinfo.php">'.$LANG["view_info"].'</a>';
			$message = '<div class="alert alert-info alert-dismissable mt-5">
					  <button type="button" class="close" data-dismiss="alert">&times;</button>
					 <i class="fa fa-refresh" ></i>  '.$numberUpdate.' '.$LANG["update_is_are_available_for_this_website"] .' 
					</div>'.$btn;
		}
		?>
