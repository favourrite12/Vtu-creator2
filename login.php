  ...
  $webLink = $webConfig["webLink"];
    $doTo = xcape($conn, $_GET['do_to']);
     if(!empty($_GET["do_to"])){
      ...
      if(!empty($_COOKIE['uKey'])) {
                $email = mysqli_real_escape_string($conn,$_COOKIE['uKey']);
                $password = $_COOKIE['uToken'];
                      $sql = "SELECT user_name,email,password,id,status,block_reason FROM users  WHERE id = '$email' ";
                      $result = $conn->query($sql);
                      if ($result->num_rows > 0) {
                      while($row = $result->fetch_assoc()) {
                if(md5(base64_encode($_SERVER['HTTP_USER_AGENT'].$row["password"])) == $_COOKIE['uOpKey']){
                      if($row["status"]=="block"){
                      ...
                      if($webConfig["allowCookie"] == 1){
                              ...
                              $uOpKey = "uOpKey";
                               ...
                               setcookie($uOpKey, $bKey, time() + (86400 * 365), "/",$webLink); // 86400 = 1 day
                        }
     ...
     ?>
      <?php
       ...
       if($go==1){
       $sql = "SELECT  user_name,email,password,id,status,block_reason FROM users  WHERE email = '$email' OR phone = '$email' OR user_name = '$email' OR id = '$email' ";
       $result = $conn->query($sql);
       if ($result->num_rows > 0) {
       while($row = $result->fetch_assoc()) {
       if($row["status"]=="block"){
       ...
       if($autoLogin=="1"){
        ...
        echo "<script> location.href='$doTo' </script>";
           ...
           include_once  '../include/header.php';
           alertDanger($LANG["user_not_found"]);
       }
       }
        }
       ?>					
	<?php include_once  '../include/header.php';?>				
	<title><?php echo $LANG["user_login"]; ?> | <?php echo $webConfig["webName"]?></title> 	
         <section id="content">
        <div id="scroll"></div>
            ...
            <div class="section flexbox">
                  ...
                  <div class="col s12 m12 l6 custom-form-control">
                    <div class="card-panel hoverable">
                        <strong><?php echo $LANG['user_login']; ?> </p></strong>
                      <div class="row">
	   ...
	   <?php if($webConfig["allowCookie"] == 1){
		...
		<div class="col s12 ">	
			...
			<div class="col s12 " style="clear:both !important">
				 ...
				 <a  href="register.php?do_to=<?php echo base64_encode($doTo);?>"><?php echo $LANG["register"];?></a>
