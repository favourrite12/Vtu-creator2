...
$result = $conn->query($sql);
    ...
    $stat[date("Y",$row["time"])] [date("n",$row["time"])] [date("j",$row["time"])] =  $stat[date("Y",$row["time"])] [date("n",$row["time"])][date("j",$row["time"])] + 1;
	}
	}else{
...
<script>
  ...
  <title><?php echo $LANG["statistic_of_visitors"] ;?></title>
